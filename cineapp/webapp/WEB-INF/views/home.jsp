   ...
   <jsp:include page="includes/menu.jsp"></jsp:include>
    ...
    ================================================== -->
      <div id="myCarousel" class="carousel slide" data-ride="carousel">
        ...
        <c:forEach items= "${ bannerList }" varStatus="l">
        	<c:choose>
				<c:when test="${ l.index==0 }">
					<li data-target="#myCarousel" data-slide-to="${l.index }" class="active"></li>
				</c:when>
				<c:otherwise>
					<li data-target="#myCarousel" data-slide-to="${ l.index }" ></li>
				</c:otherwise>  
			</c:choose>  							     	
      	</c:forEach>
      	</ol>
          	...
          	<c:choose>
				<c:when test="${ loop.index==0 }">
					<div class="item active">         
			            <img src="${ urlPublic }/images/${ banner.archivo }" alt="${ banner.archivo }" title="${ banner.titulo }" >
			        </div>
				</c:when>
				<c:otherwise>
					<div class="item">         
            ...
            <div class="form-group">
              <label for="fecha">Fecha: </label>
	              <select id="fecha" name="fecha" class="form-control">      
	              	<c:forEach items="${ listaFechas }" var="fechas">
	              		<c:choose>
	              			<c:when test="${ fechas == fechaBusqueda}">
